...
router.get('/export', (req, res) => {
  ...
  Media.find(query).sort({ createdAt: -1 }).exec((err, medias) => {
    if (err) {
      ...
      const specification = {
        ...
        authors: {
          ...
          cellFormat (value, row) {
            return value.map(authors => [authors.first_name, authors.last_name].filter(name => name).join(' ')).join(', ')
          }
        },
        publishing_date: {
        ...
        organizers: {
          ...
          headerStyle: {},
          cellFormat (value, row) {
            return value.map(organizer => [organizer.first_name, organizer.last_name].filter(name => name).join(' ')).join(', ')
          }
        },
        city: {
          displayName: 'Cidade',
        ...
        additional_infos: {
          ...
          headerStyle: {},
          cellFormat (value) {
            return value.filter(info => info.label && info.value).map(info =>
              ([info.label, info.value].join(': '))).join(', ')
          }
        },
        url: {
          displayName: 'URL',
        ...
        docs: {
          ...
          cellFormat (value) {
            return value
              ...
              .map(document => formatURL(document.url)).join(', ')
          }
        },
        image: {
